...
class extMain extends Main {
	...
	function image_check() {
		...
		foreach ($this->files as $file) { // extract files hasil pencarian
			...
			if(! in_array($image_type, array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG, IMAGETYPE_BMP, IMAGETYPE_ICO))) { // pembanding gambar asli dengan yang palsu
	...
	function updatedb() {
		...
		foreach ($this->files as $file) { // extract hasil pencarian menjadi satuan
			...
			$dbcheck = mysql_fetch_row(mysql_query("SELECT md5sum, last_modified FROM data_file WHERE files='$file'")); // fetch md5 dan last time modified dari database
			switch(True):
